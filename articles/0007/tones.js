TONES = (function(exports) {
    ...
    function playNote(frequency = 440, start = 0, end = 1) {
        var oscillator = audioContext.createOscillator();
        var oscillatorGainNode = audioContext.createGain();

        var curr = audioContext.currentTime;
        ...
        oscillatorGainNode.gain.setTargetAtTime(0, curr + end - 3 * padding, padding);

        oscillator.stop(curr + end + padding);
    }

    function setAudioContextTimeout(audioCtxTime, func) {
        var udderlynode = audioContext.createOscillator();
        var curr = audioContext.currentTime;

        udderlynode.start(curr + audioCtxTime);
        udderlynode.stop(curr + audioCtxTime);
        udderlynode.onended = func;

        udderlynode.connect(audioContext.destination);
    }
    ...
    exports.notes = Notes;
    exports.timeout = setAudioContextTimeout;
    exports.setNoteColor = setNoteColor;
    exports.audioCtx = audioContext;
